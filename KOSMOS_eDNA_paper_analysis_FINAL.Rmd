...
peru_map <- ggplot(data = world_df, aes(x = long, y = lat, group = group))+
  ...
  annotate("text", x = -82.5, y = -21.5,label = "Pacific", color = "darkblue",size = 6, parse = TRUE) # Pacific

peru_map

ggsave("kosmos_map.pdf", width = 5, height = 8,path = here::here("figures", "minor_revisions"))
```
...
# Add points for sampling points
  ...
  # Three different themes: for top panel, middle three panels, and bottom panel
    theme(panel.background = element_rect(fill="white"),
        ...
        axis.text.x.bottom = element_text(size = ifelse(panel[i] %in% c("(e)"),20,0)),
        axis.text.x.top = element_text(size = ifelse(panel[i] %in% c("(a)"),20,0)),
        axis.title.x.bottom = element_text(size = ifelse(panel[i] %in% c("(e)"),25,0)),
        axis.title.x.top = element_text(size = ifelse(panel[i] %in% c("(a)"),25,0)),
        axis.title.y = element_text(size = 20,margin = margin(0, ifelse(panel[i] %in% c("(b)"), 1.7, 
  ...
  ylab(ifelse(panel[i] == "(a)",expression(~"Temperature" ~ ("°C")),
              ifelse(panel[i] == "(b)",expression(~"Salinity" ~ ("psu")),
                     ifelse(panel[i] == "(c)",expression(~"NO"[2] ~ + ~"NO"[3] ~ (mu*"mol L" ^-1)~"\n"),
                            ifelse(panel[i] == "(d)",expression(~"Si(OH)"[4] ~ (mu*"mol L" ^-1)~"\n"),
                                   ifelse(panel[i] == "(e)",expression(~"chl-a" ~ (mu*"g L" ^-1) ~ "\n"),
              "oops you messed up"))))))

ggsave(here::here("figures", "minor_revisions", "biogeochemical_figure", paste0(variable_names[i],"_gg.png")) ,plot,width = 7, height = ifelse(panel[i] %in% c("(a)","(e)"),3.5,3))
...
ggsave("legend_gg.png", width = 7, height = 1,path = here::here("figures", "minor_revisions", "biogeochemical_figure"))
...
ggsave("legend_final.png", width = 11, height = 1,path = here::here("figures", "minor_revisions", "biogeochemical_figure"))
```
...
# Add points for sampling points
  ...
  geom_point(data = eval(parse(text = paste0(variable_names[i],"_df"))), aes(x = sampling.day, y = Pacific),color = "black",size = 1.5)+
    ...
    theme(panel.background = element_rect(fill="white"),
          panel.border = element_rect(colour = "black", fill=NA, size=2),
          axis.text.y = element_text(size = 15),
          axis.text.x.bottom = element_text(size = ifelse(panel[i] %in% c("(j)"),20,0)),
          axis.text.x.top = element_text(size = ifelse(panel[i] %in% c("(f)"),20,0)),
          axis.title.x.bottom = element_text(size = ifelse(panel[i] %in% c("(j)"),25,0)),
          axis.title.x.top = element_text(size = ifelse(panel[i] %in% c("(f)"),25,0)),
          ...
          axis.title.y = element_text(size = 20,margin = margin(t = ifelse(panel[i] == "(f)",0,5),r = ifelse(panel[i] %in% c("(f)"),12.5,
                                                                        ifelse(panel[i] %in% c("(g)"),21,
                                                                               ifelse(panel[i] %in% c("(h)"),0,
                                                                                    ifelse(panel[i] %in% c("(i)"),5,
    ...
    geom_vline(xintercept = 33, lty = 2, color = "#ff7f00", size = 1)+
    # Use an ifelse statement to change the y labels
    ylab(ifelse(panel[i] == "(f)",expression(italic(~"Synechococcus")),
                ifelse(panel[i] == "(g)",expression(~"Total eukaryotes"),
                       ifelse(panel[i] == "(h)",expression(~"Cryptophytes"),
                              ifelse(panel[i] == "(i)",expression(~"Heterotrophic bacteria"),
                                     ifelse(panel[i] == "(j)",expression(~"Small bacteria"),
                                            "oops you messed up"))))))
  
  ggsave(here::here("figures", "minor_revisions", "biogeochemical_figure", paste0(variable_names[i],"_gg.png")),plot,width = 7, height = ifelse(panel[i] %in% c("(f)","(j)"),3.5,3))
}

# Make a "cells/ml" label
cells_ml_legend_gg <- ggplot(data = syn_df, aes(x = sampling.day, y = M1))+
         geom_point()+
         ylab(expression(~"Cells ml" ^-1 ~ ("x 10"^4) ~ "\n"))+
  theme(axis.title.y = element_text(size = 30))

ggsave(here::here("figures", "minor_revisions", "biogeochemical_figure", "cells_ml_legend.png"), cells_ml_legend_gg,width = 7,height = 7)
```
...
ggsave(here::here("figures", "minor_revisions", "PCA_figure", "PCA_legend_v2.png"),legend_gg, width = 7, height = 2)
...
ggsave(here::here("figures", "minor_revisions", "PCA_figure", "COI_pca_kosmos_3PCs_16samples_9.25.20.png"),pca_gg_sample,width = 7,height = 6)
...
ggsave(here::here("figures", "minor_revisions", "PCA_figure", "18S_pca_kosmos_3PCs_16samples_9.25.20.png"),pca_gg_sample,width = 7,height = 6)
...
ggsave(here::here("figures", "minor_revisions", "PCA_figure", "12S_pca_kosmos_3PCs_16samples_9.25.20.png"),pca_gg_sample,width = 7,height = 5)
```
...
ggsave(here::here("figures", "minor_revisions", "PCA_figure", "12S_pca_kosmos_chordata_3PCs_16samples_9.25.20.png"),pca_gg_sample,width = 7,height = 6)
```
...
ggsave(here::here("figures", "minor_revisions", "PCA_figure", "16S_plastidial_pca_kosmos_3PCs_16samples_9.25.20.png"),pca_gg_sample,width = 7,height = 6)
...
ggsave(here::here("figures", "minor_revisions", "PCA_figure", "16S_bacterial_pca_kosmos_3PCs_16samples_9.25.20.png"),pca_gg_sample,width = 7,height = 6)
...
ggsave(here::here("figures", "minor_revisions", "PCA_figure", "combined_figure_legend_top30_092520.png"),legend_gg,width = 21, height = 5)
...
# Manually change sign of PC1 in loadings (to match how we flipped them in the PCA plot)
PC_loadings_COI$PC1 <- PC_loadings_COI$PC1*-1
...
COI_PCA_gg <- ggplot(PC_loadings_COI_30,aes(x = reorder(FeatureID,PC1),y = PC1, fill = group))+
  geom_bar(stat = "identity")+
  coord_flip()+
  geom_text(data = PC_loadings_COI_30,aes(x = reorder(FeatureID,PC1),y = ifelse(PC1 > 0, -0.01,0.01), label = taxonomy, hjust = ifelse(PC1 > 0, 1,0),color = ifelse(taxonomy =="Unassigned","gray60","black"),fontface = ifelse(!(Genus %in% c("g_","unassigned","no_hit")),"italic","plain")), size = 5)+
  ylim(-0.21,0.21)+
...
ggsave(here::here("figures", "minor_revisions", "PCA_figure", "COI_PC1_unmerged_top30_072720.png"),COI_PCA_gg,width = 7, height = 8.5)
```
...
# Manually change sign of PC1 in loadings (to match how we flipped them in the PCA plot)
PC_loadings_18S$PC1 <- PC_loadings_18S$PC1*-1
                                                                                            ...
                                                                                            ifelse(Family %in% c("Vexilliferidae"),"Amoebozoa",
                                                                                                   ifelse(Family %in% c("Paulinellidae") | Genus == "Protaspis","Cercozoa",
                                                                                                          ifelse(Family %in% c("Pycnococcaceae"),"Chlorophyta",
                                                                                                                 ...
                                                                                                                 # Chloroplast       
                                                                                                                 ifelse(Class %in% c("Cryptophyceae") | Phylum %in% c("Cryptophyta"),"Cryptophytes",
                                                                                                                        ifelse(Class %in% c("Embryophyceae"), "Embryophytes",
                                                                                                                               ifelse(Class %in% c("Dictyophyceae"), "Silicoflagellates",
                                                                                                                                      ifelse(Family %in% c("Chrysochromulinaceae"),"Prymnesiales",
                                                                                                                                             ifelse(Class %in% c("Trebouxiophyceae"),"Chlorophyta",
...
# Subset the ASV table based on a relative abundance threshold in multiple samples
# The problem with this approach is that it can eliminate taxa that don't have a lot of reads because of amplification bias, like Akashiwo
## Set threshold to at least 0.001 in at least 4 samples -> gives us 90 ASVs. Totally manageable. This is loose enough to keep Akashiwo in the picture.
# Add a column to the normalized asv table to store number of values over our cutoff (in this case 0.001)
...
PCA_gg_18S <- ggplot(PC_loadings_18S_30,aes(x = reorder(FeatureID,PC1),y = PC1, fill = group))+
  geom_bar(stat = "identity")+
  coord_flip()+
  geom_text(data = PC_loadings_18S_30,aes(x = reorder(FeatureID,PC1),y = ifelse(PC1 > 0, -0.01,0.01), label = taxonomy, hjust = ifelse(PC1 > 0, 1,0),color = ifelse(taxonomy =="Unassigned","gray60","black"),fontface = ifelse(!(Genus %in% c("g_","unassigned","no_hit")),"italic","plain")), size = 5)+
  ylim(-0.21,0.21)+
  theme(axis.text.y = element_blank(),
        axis.title.y = element_blank(),
        axis.ticks.length.y=unit(0, "cm"),
...
ggsave(here::here("figures", "minor_revisions", "PCA_figure", "18S_PC1_unmerged_top30_092520.png"),PCA_gg_18S,width = 7, height = 8.5)
```
...
chloroplast_PCA_gg <- ggplot(PC_loadings_chloroplast_30,aes(x = reorder(FeatureID,PC1),y = PC1, fill = group))+
  geom_bar(stat = "identity")+
  coord_flip()+
  geom_text(data = PC_loadings_chloroplast_30,aes(x = reorder(FeatureID,PC1),y = ifelse(PC1 > 0, -0.01,0.01), label = taxonomy, hjust = ifelse(PC1 > 0, 1,0),color = ifelse(taxonomy =="Unassigned","gray60","black")), size = 5)+
  ylim(-0.4,0.4)+
  theme(axis.text.y = element_blank(),
        axis.title.y = element_blank(),
        axis.ticks.length.y=unit(0, "cm"),
...
ggsave(here::here("figures", "minor_revisions", "PCA_figure", "16S_plastidial_PC1_unmerged_top30_092520.png"),chloroplast_PCA_gg,width = 7, height = 8.5)
```
...
ggsave(here::here("figures", "minor_revisions", "PCA_figure", "16S_bacterial_PC1_unmerged_top30_100520.png"),bacteria_PCA_gg,width = 7, height = 8.5)
```
...
ggsave(here::here("figures", "minor_revisions", "12S", "12S_sample_sums.png"),kosmos_12S_sample_sums_gg,width = 10, height = 6)
```
...
ggsave(here::here("figures", "minor_revisions", "12S", "12S_mesocosm_barplot_no_paralichthys_genus_plus_birds_090220.png"),kosmos_12S_barplot,width = 8, height = 3.5)
```
...
ggsave(here::here("figures", "minor_revisions", "12S", "12S_pacific_barplot_noparalichthys_genus.png"),kosmos_12S_barplot,width = 8, height = 3.5)
```
...
ggsave(here::here("figures", "minor_revisions", "12S", "12S_genus_barplot_legend.png"),legend_gg,width = 8, height = 2)
```
...
ggsave(here::here("figures", "minor_revisions", "12S", "paralichthys_M1.png"),paralichthys_M1_gg,width = 8, height = 4)
```